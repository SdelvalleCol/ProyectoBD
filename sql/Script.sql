...
DROP TRIGGER IF EXISTS ACTUALIZACION_VEHICULOS_ALQUILER;
DELIMITER $$
...
BEGIN
	SELECT ESTADOS_VEHICULO_ID_ESTADO INTO @ESTADO_VEHICULO_INGRESADO FROM VEHICULOS, ALQUILERES_has_VEHICULOS WHERE VEHICULOS_ID_VEHICULO = ID_VEHICULO;
	IF @ESTADO_VEHICULO_INGRESADO = 1 THEN
        ...
        SELECT VALOR_DIA INTO @VALOR_DIA_VEHICULO FROM VEHICULOS, ALQUILERES_has_VEHICULOS WHERE ID_VEHICULO = VEHICULOS_ID_VEHICULO;
        SELECT VALOR_DIA, DATEDIFF(FECHA_LLEGADA, FECHA_SALIDA), PORCENTAJE_DESCUENTO INTO @VALOR_DIA_ALQUILER, @CANTIDAD_DIAS_ALQUILER, @VALOR_PORCENTAJE 
        FROM ALQUILERES, ALQUILERES_has_VEHICULOS WHERE ID_ALQUILER = ALQUILERES_ID_ALQUILER;
...
INSERT INTO VEHICULOS(PLACA, REFERENCIA, MODELO, PUERTAS, CAPACIDAD, SUNROOF, MOTOR, VALOR_DIA, SUCURSALES_ID_SUCURSAL, TIPOS_ID_TIPO, ESTADOS_VEHICULO_ID_ESTADO, MARCAS_VEHICULO_ID_MARCA)
VALUES(@FIELD_PLACA, @FIELD_REFERENCIA, @FIELD_MODELO, @FIELD_PUERTAS, @FIELD_CAPACIDAD, @FIELD_SUNROOF, @FIELD_MOTOR, @FIELD_VALOR_DIA, @FIELD_ID_SUCURSAL, @FIELD_ID_TIPO, @FIELD_ESTADO_VEHICULO, @FIELD_MARCA_VEHICULO);
...
INSERT INTO PERSONAS(CEDULA, NOMBRES, APELLIDOS, DIRECCION, CELULAR, CORREO_ELECTRONICO, CONTRASENA, CIUDADES_ID_CIUDAD)
...
SET @FIELD_CONTRASENA = 'emplErnesto2022*/';
SET @FIELD_CIUDAD = 2;
...
INSERT INTO ALQUILERES (ID_SUCURSAL_SALIDA, FECHA_SALIDA, ID_SUCURSAL_LLEGADA, FECHA_LLEGADA, VALOR_SEMANA, VALOR_DIA, PORCENTAJE_DESCUENTO, VALOR_COTIZADO, VALOR_PAGADO, RESUELTO, EMPLEADOS_ID_EMPLEADO, CLIENTES_ID_CLIENTES)
VALUES (@FIELD_ID_SUCURSAL_SALIDA, @FIELD_FECHA_SALIDA, @FIELD_ID_SUCURSAL_LLEGADA, @FIELD_FECHA_LLEGADA, @FIELD_VALOR_SEMANA, @FIELD_VALOR_DIA, @FIELD_PORCENTAJE_DESCUENTO, @FIELD_VALOR_COTIZADO, @FIELD_VALOR_PAGADO, @FIELD_RESUELTO, @FIELD_ID_EMPLEADO_ENCARGADO, @FIELD_ID_CLIENTE);
...
SET @SQLEMPLEADOLOGIN = 'SELECT * FROM PERSONAS, EMPLEADOS WHERE CORREO_ELECTRONICO=? AND CONTRASENA=? AND ID_PERSONA = PERSONAS_ID_PERSONA';
...
SET @SQLCLIENTELOGIN = 'SELECT * FROM PERSONAS, CLIENTES WHERE CORREO_ELECTRONICO=? AND CONTRASENA=? AND ID_PERSONA = PERSONAS_ID_PERSONA';
...
SET @FIELD_TIPO_VEHICULO_CONSULTA = 1;
